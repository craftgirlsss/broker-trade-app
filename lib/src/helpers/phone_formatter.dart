...
class InternationalPhoneFormatter extends TextInputFormatter {
  ...
  String internationalPhoneFormatted = nums.length >= 1
    ? '+${nums.substring(0, nums.length >= 1 ? 1 : null)}${nums.length  > 1 ? ' (' : ''}${nums.substring(1, nums.length >= 4 ? 4 : null)}${nums.length  > 4 ? ') ' : ''}${nums.length > 4
      ? nums.substring(4, nums.length >= 7 ? 7 : null) + (nums.length > 7
        ? '-${nums.substring(7, nums.length >= 12 ? 12 : null)}'
        : '')
      : ''}'
    : nums;
